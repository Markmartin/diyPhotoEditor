<template>
  <div
    ...
    @mousedown.stop.prevent="bodyMouseDown($event)"
    @touchstart.stop.prevent="bodyMouseDown($event)"
  >
    <div :style="{ transform: `scaleX(${mirror})` }">
      <slot></slot>
    </div>
...
export default {
  ...
  computed: {
    sticks() {
      let sticks = []
      // if (this.resizable) sticks.push('tl', 'tr', 'br', 'bl')
      if (this.resizable) sticks.push('br')
      if (this.rotatable) sticks.push('ro')
      return sticks
    },
    classObject() {
      return {
    ...
    hasActiveContent: {
      handler: function(val) {
        if (val) {
          ...
          if (this.contentActive) this.onContentInactive()
        }
      },
      immediate: true
    },
  ...
  methods: {
    ...
    bodyMove(ev) {
      ...
      if (this.dragHandler) this.setRect(this.dragHandler(this.getRect()))
    ...
    stickDown: function(stick, ev) {
      if (!this.resizable || !this.active) return
    ...
    stickMove(ev) {
      ...
      if (this.currentStick == 'ro') {
        ...
        if (this.resizeHandler) this.setRect(this.resizeHandler(this.getRect()))
